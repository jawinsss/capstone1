...
class UserAPI {
  ...
  async getProvinceName(code) {
    try {
      const response = await this.apiService.get(
        `/locations/provinces?search=${code}&limit=1`
      );
  ...
  async getWardName(wardCode, provinceCode) {
    try {
      const response = await this.apiService.get(
        `/locations/wards/${provinceCode}?search=${wardCode}&limit=1`
      );
      if (response.success && response.data.length > 0) {
        return response.data[0].name_with_type || response.data[0].name;
      }
...
async function updateProfileUI(user) {
  ...
  if (user.ward || user.wardName) {
    ...
    if (wardDropdown && user.province) {
      // Update ward API URL to use VietMap for the selected province
      wardDropdown.options.apiUrl = `vietmap://wards/${user.province}`;
  ...
  if (user.street) {
    ...
    // Set street data globally for dropdown
    ...
    // Update street dropdown display if it exists
    const streetDropdown = window.streetDropdown;
    if (streetDropdown) {
      streetDropdown.setValue(user.street);
      // Mark as valid selection
      if (streetDropdown.input) {
        streetDropdown.input.dataset.hasValidSelection = "true";
        streetDropdown.input.dataset.selectedLat = user.latitude || 0;
        streetDropdown.input.dataset.selectedLng = user.longitude || 0;
      }
      console.log("✅ Street dropdown updated with GPS:", {
        street: user.street,
        lat: user.latitude,
        lng: user.longitude,
      });
    } else {
      console.warn("⚠️ Street dropdown not ready yet");
    }
  }
...
function applyOrderFilters() {
  ...
  if (userOrdersState.filters.status) {
    filtered = filtered.filter(
      (o) => o.status === userOrdersState.filters.status
    );
  }
...
function renderOrders() {
  ...
  const sorted = [...userOrdersState.filteredOrders].sort(
...
async function handleViewOrderDetail(orderId) {
    ...
    .map((item) => {
      ...
      return `
            <div class="order-product-item" style="margin-bottom: 12px;">
                ...
                <div class="order-product-info">
                    <div class="order-product-name">${name}</div>
                    <div class="order-product-price">${formatVND(
                      price
                    )} × ${qty}</div>
                </div>
  ...
  modalBody.innerHTML = `
        ...
        <div class="order-detail-section" style="margin-bottom: 24px;">
            <h4><i class="fa-solid fa-box"></i> Sản phẩm đã đặt (${
...
async function handleCancelOrder(orderId) {
  ...
  try {
    const response = await userAPI.apiService.patch(
...
async function submitReturnRequest() {
  ...
  try {
    ...
    const response = await userAPI.apiService.post(
    ...
    const actualResponse =
      response.data && response.data.success ? response.data : response;
...
function updateNotificationsUI(notifications) {
  ...
  if (unreadCount > 0) {
    ...
    alert.querySelector(
      "i"
    ).nextSibling.textContent = `Bạn có ${unreadCount} thông báo chưa đọc`;
  } else {
  ...
  notificationDiv.innerHTML = `
      ...
      <div class="notification-content">
        ...
        <div class="notification-actions">
          ${
            !notification.isRead
              ? '<button class="btn-primary" onclick="markAsRead(this)">Đánh dấu đã đọc</button>'
              : '<button class="btn-disabled" disabled>Đã đọc</button>'
          }
          <button class="btn-secondary" onclick="handleNotificationAction('${
            notification.type
          }')">Chi tiết</button>
        </div>
      </div>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const waitForDropdowns = setInterval(async () => {
    if (window.dropdownsReady) {
      ...
      try {
        ...
        if (
          activeSection &&
          activeSection.getAttribute("data-section") === "orders"
        ) {
  ...
  window.updateAvatarInitials = function () {
    ...
    const span =
      document.querySelector("#profileAvatarPlaceholder #avatarInitial") ||
      document.querySelector("#profileAvatarPlaceholder span");
  ...
  function applyAvatarData(dataUrl) {
    if (dataUrl) {
      ...
      const initials = window.getInitials(
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const streetContainer = document.getElementById("streetSearchContainer");
  ...
  if (!provinceContainer || !wardContainer || !streetContainer) {
    console.warn(
      "Address form elements not found, skipping address initialization"
    );
    return;
  }
  ...
  function updateCombinedAddress() {
    ...
    if (addressLat && selectedStreet)
      addressLat.value = selectedStreet.lat || 0;
    if (addressLng && selectedStreet)
      addressLng.value = selectedStreet.lng || 0;
  ...
  // Initialize province search dropdown with VietMap API
  const provinceDropdown = new SearchDropdown(provinceContainer, {
    placeholder: "Chọn tỉnh/thành phố...",
    apiUrl: "vietmap://provinces", // Use VietMap service
    limit: 999, // Load all provinces at once
    showLoadMore: false, // Disable load more button
    customFetch: async (url, search, page, limit) => {
      // Use VietMap service instead of backend API
      if (window.vietMapService) {
        return await window.vietMapService.getProvinces(search, page, limit);
      }
      return {
        success: false,
        data: [],
        message: "VietMap service not available",
      };
    },
    onSelect: (province) => {
      ...
      streetDropdown.clearSearch();
      streetDropdown.options.disabled = true; // Disable until ward is selected
  ...
  console.log("✅ Province dropdown created with VietMap");
  ...
  const wardDropdown = new SearchDropdown(wardContainer, {
    ...
    customFetch: async (url, search, page, limit) => {
      // Use VietMap service instead of backend API
      if (window.vietMapService && selectedProvince) {
        return await window.vietMapService.getWards(
      ...
      return {
        success: false,
        data: [],
        message: "VietMap service not available or no province selected",
      };
    },
    onSelect: (ward) => {
      ...
      // Enable street dropdown after ward is selected
      if (streetDropdown.options.disabled) {
        streetDropdown.options.disabled = false;
        streetDropdown.enable();
      }
  ...
  console.log("✅ Ward dropdown created with VietMap");

  // Initialize street autocomplete dropdown with VietMap API
  const streetDropdown = new SearchDropdown(streetContainer, {
    placeholder: "Nhập số nhà, tên đường... (ví dụ: 123 Nguyễn Huệ)",
    apiUrl: "vietmap://streets", // Use VietMap service
    limit: 10,
    showLoadMore: false,
    disabled: true, // Disabled until ward is selected
    required: true, // MUST select from dropdown
    customFetch: async (url, search, page, limit) => {
      // Use VietMap address autocomplete service with GPS
      // MUST have both province AND ward selected for accurate GPS
      if (
        window.vietMapService &&
        selectedProvince &&
        selectedWard &&
        search &&
        search.trim().length >= 2
      ) {
        const wardId = selectedWard.id || selectedWard.code;
        console.log(
          `🔍 Searching streets in ${selectedProvince.name}, Ward ${wardId}: "${search}"`
        );
        return await window.vietMapService.autocompleteAddress(
          search,
          selectedProvince.id,
          wardId,
          page,
      ...
      // Show error if ward not selected
      if (!selectedWard) {
        return {
          success: false,
          data: [],
          message: "Vui lòng chọn Phường/Xã trước khi nhập địa chỉ",
        };
      }
      return {
        success: false,
    ...
    onSelect: (street) => {
      console.log("🛣️ Street selected with GPS:", street);
      selectedStreet = street;
      window.selectedStreet = street; // Store globally for saveChanges
      // Mark as valid selection
      if (streetDropdown.input) {
        streetDropdown.input.dataset.hasValidSelection = "true";
        streetDropdown.input.dataset.selectedLat = street.lat || 0;
        streetDropdown.input.dataset.selectedLng = street.lng || 0;
      }
      updateCombinedAddress();
    },
  });
  ...
  // Store dropdown references globally
  window.streetDropdown = streetDropdown;
  console.log("✅ Street dropdown created with VietMap GPS autocomplete");

  // Listen to input changes on street dropdown to invalidate manual typing
  if (streetDropdown.input) {
    streetDropdown.input.addEventListener("input", (e) => {
      // If user types manually (not selecting from dropdown), mark as invalid
      if (!e.target.dataset.isSelectingFromDropdown) {
        delete e.target.dataset.hasValidSelection;
        delete e.target.dataset.selectedLat;
        delete e.target.dataset.selectedLng;
        selectedStreet = null;
        window.selectedStreet = null;
        console.log("⚠️ Manual typing detected - selection invalidated");
      }
    });
  }
  ...
  provinceDropdown.options.onSelect = (province) => {
    ...
    wardDropdown.clearSearch();
    streetDropdown.clearSearch();
    streetDropdown.options.disabled = true;
    streetDropdown.disable();

    // Update ward dropdown to use VietMap with selected province
    wardDropdown.options.apiUrl = `vietmap://wards/${province.code}`;
  ...
  setTimeout(() => {
    ...
    if (window.streetDropdown) {
      window.streetDropdown.disable();
      // Also disable the input element directly
      if (window.streetDropdown.input) {
        window.streetDropdown.input.setAttribute("readonly", true);
        window.streetDropdown.input.style.background = "var(--gray-100)";
        window.streetDropdown.input.style.color = "var(--muted)";
        window.streetDropdown.input.style.cursor = "not-allowed";
      }
    }
    ...
    console.log(
      "✅ Address dropdowns initialized and disabled, ready for data"
    );
  }, 150);
});
...
function storeOriginalValues() {
  ...
  editableFields.forEach((field) => {
    ...
    if (element) {
      ...
      console.warn(
        `Element with id '${field.id}' not found when storing original values`
      );
...
function exitEditMode() {
  ...
  document
    ...
    .removeEventListener("input", updateDisplayName);
...
function toggleEditMode() {
  ...
  if (isEditing) {
    ...
    if (window.provinceDropdown) {
      ...
      console.log(
        "📍 Province dropdown enabled with data:",
        window.selectedProvince
      );
      console.log(
        "📍 Province dropdown input disabled?",
        window.provinceDropdown.input?.disabled
      );
    } else {
    ...
    if (window.wardDropdown) {
      ...
      console.log(
        "📍 Ward dropdown input disabled?",
        window.wardDropdown.input?.disabled
      );
    } else {
      console.error("❌ Ward dropdown not found!");
    }
    // Enable street dropdown ONLY if ward is selected
    if (window.streetDropdown && window.selectedWard) {
      window.streetDropdown.enable();
      // Remove readonly attribute and enable styling
      if (window.streetDropdown.input) {
        window.streetDropdown.input.removeAttribute("readonly");
        window.streetDropdown.input.style.background = "white";
        window.streetDropdown.input.style.color = "var(--text)";
        window.streetDropdown.input.style.cursor = "text";
      }
      console.log("📍 Street dropdown enabled for editing");
    }
...
async function saveChanges() {
  ...
  const selectedStreet = window.selectedStreet || null;

  // CRITICAL VALIDATION: Check if street has GPS coordinates
  const streetInput = window.streetDropdown?.input;
  const hasValidStreetSelection =
    streetInput?.dataset?.hasValidSelection === "true";
  const streetLat = parseFloat(streetInput?.dataset?.selectedLat || 0);
  const streetLng = parseFloat(streetInput?.dataset?.selectedLng || 0);

  console.log("🔍 Street validation:", {
    streetInputValue: streetInput?.value,
    hasValidSelection: hasValidStreetSelection,
    streetLat,
    streetLng,
    selectedStreet,
  });

  // If user typed in street field but didn't select from dropdown with GPS
  if (streetInput && streetInput.value.trim() && !hasValidStreetSelection) {
    showModal(
      "⚠️ Lỗi địa chỉ",
      "Bạn phải chọn địa chỉ từ danh sách gợi ý (có GPS).\n\nKhông được nhập tùy tiện!\n\nVui lòng nhập số nhà/tên đường và chọn từ dropdown.",
      null,
      null
    );
    return;
  ...
  // Skip GPS validation - some addresses may have 0,0 coordinates which is valid
  // The important check is hasValidSelection above

  // Build full address
  const addressParts = [];
  if (addressStreet) addressParts.push(addressStreet);
  if (selectedWard)
    addressParts.push(selectedWard.name_with_type || selectedWard.name);
  if (selectedProvince)
    addressParts.push(selectedProvince.name_with_type || selectedProvince.name);
    ...
    async () => {
      try {
        ...
        const updateData = {
          ...
          latitude: streetLat || 0, // Add GPS coordinates
          longitude: streetLng || 0, // Add GPS coordinates
        };
      ...
      editableFields.forEach((field) => {
        ...
        if (element && originalValues[field.id] !== undefined) {
          try {
            ...
            console.error(
              `Error restoring value for field '${field.id}':`,
              error
            );
          }
        } else if (!element) {
          console.warn(
            `Element with id '${field.id}' not found when restoring values`
          );
        }
      });
...
function restoreNotificationSettings() {
  document.getElementById("orderStatus").checked =
    originalNotificationSettings.orderStatus;
  document.getElementById("paymentNotif").checked =
    originalNotificationSettings.paymentNotif;
  document.getElementById("promotions").checked =
    originalNotificationSettings.promotions;
  document.getElementById("newProducts").checked =
    originalNotificationSettings.newProducts;
  document.getElementById("emailNotif").checked =
    originalNotificationSettings.emailNotif;
  document.getElementById("smsNotif").checked =
    originalNotificationSettings.smsNotif;
}
...
document
  .getElementById("changeRecoveryEmail")
