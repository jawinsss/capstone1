...
class SearchDropdown {
  constructor(container, options = {}) {
    ...
    this.options = {
      ...
      customFetch: null, // Custom fetch function for API calls
      onSelect: () => {},
      onLoadMore: () => {},
      ...options,
    };
  ...
  async loadData(query = "", page = 1) {
    ...
    // Skip if no API URL is set and no custom fetch function
    if (!this.options.apiUrl && !this.options.customFetch) {
      this.showNoResults();
      return;
    }
    ...
    try {
      ...
      if (this.options.customFetch) {
        data = await this.options.customFetch(
          this.options.apiUrl,
          query,
          page,
          this.options.limit
  ...
  renderItems() {
    ...
    const loadMoreHtml =
      this.options.showLoadMore && this.hasMore
        ? `
    ...
    const loadMoreBtn = this.list.querySelector(
      ".search-dropdown-load-more button"
    );
    if (loadMoreBtn) {
  ...
  showLoading() {
    this.list.innerHTML =
      '<div class="search-dropdown-loading">Đang tải...</div>';
    this.showList();
  }

  showNoResults() {
    const message =
      !this.options.apiUrl && !this.options.customFetch
